...
const api = await ApiPromise.create({
  provider: new WsProvider("wss://rpc.dbcwallet.io"),
});
await api.isReady;
...
const entries = await api.query.onlineProfile.machinesInfo.entriesPaged({
  args: [],
  pageSize: 200,
});
const bare = [];
for (const [key, val] of entries) {
  ...
  const status =
    typeof m.machineStatus === "string"
      ? m.machineStatus
      : Object.keys(m.machineStatus)[0];
  if (status !== "online" && status !== "rented") continue;
  const machineId = key.args[0].toHuman();
  const rented =
    (await api.query.onlineProfile.machineRentedGpu(machineId)).toNumber?.() ??
    0;
  const free = (cu.gpuNum || 0) - rented;
  if (free <= 0) continue;
  bare.push({
    machineId,
    gpuType: hexToStr(cu.gpuType),
    gpuNum: cu.gpuNum,
    freeGpu: free,
    calcPoint: cu.calcPoint,
    stash: m.machineStash,
  });
}
console.log(bare);
...
function hexToStr(h) {
  try {
    return Buffer.from(String(h).replace(/^0x/, ""), "hex").toString("utf8");
  } catch {
    return String(h);
  }
}
...
const ONE = 10n ** 15n; // DBC has 15 decimals (1 DBC = 10^15 base units)
const sign = (api, tx, who) =>
  new Promise((res, rej) =>
    tx
      .signAndSend(who, ({ status, events, dispatchError }) => {
...
async function main() {
  ...
  const renter = new Keyring({ type: "sr25519" }).addFromMnemonic(
    process.env.RENTER_MNEMONIC,
  );
  ...
  for (const [k, v] of entries) {
    ...
    const status =
      typeof m.machineStatus === "string"
        ...
        : Object.keys(m.machineStatus)[0];
    if (
      sc.isBareMachine === true &&
      status === "online" &&
      (cu.gpuNum || 0) > 0
    ) {
      const rented =
        (
          await api.query.onlineProfile.machineRentedGpu(k.args[0].toHuman())
        ).toNumber?.() ?? 0;
      if (cu.gpuNum - rented >= 1) {
        target = k.args[0].toHuman();
        break;
      }
    }
  }
  ...
  const evs = await sign(
    api,
    api.tx.rentMachine.rentMachine(target, 1, 300),
    renter,
  );
  const rentEv = evs.find(
    (e) => e.event.section === "rentMachine" && e.event.method === "Rent",
  );
...
_本文档基于 DeepBrainChain 主网 runtime（rentMachine / onlineProfile pallet）。如 runtime 升级导致接口变化，以链上 metadata (`api.tx.rentMachine` / `api.query.onlineProfile`) 为准。_
